    ...
    if (this.state !== 'poweredOn') {
        ...
        if (serviceUuids && serviceUuids.length) {
            for (var i = 0; i < serviceUuids.length; i++) {
                undashedServiceUuids[i] = UuidUtil.removeDashes(
                    serviceUuids[i]
                );
            }
        }
...
Bleno.prototype.startAdvertisingIBeacon = function(
    uuid,
    major,
    minor,
    measuredPower,
...
Bleno.prototype.startAdvertisingWithEIRData = function(
    advertisementData,
    scanData,
    callback
) {
    ...
    if (this.state !== 'poweredOn') {
        var error = new Error(
            'Could not advertising scanning, state is ' +
                this.state +
                ' (not poweredOn)'
        );
        ...
        this._bindings.startAdvertisingWithEIRData(
            advertisementData,
            scanData
        );
    }
