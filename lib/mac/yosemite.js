...
blenoBindings.startAdvertising = function(name, serviceUuids) {
    ...
    if (serviceUuids && serviceUuids.length) {
        for (var i = 0; i < serviceUuids.length; i++) {
            advertisement.kCBAdvDataServiceUUIDs[i] = new Buffer(
                serviceUuids[i],
                'hex'
            );
        }
...
blenoBindings.startAdvertisingWithEIRData = function(advertisementData) {
    if (osRelease < 14) {
        throw new Error(
            'startAdvertisingWithEIRData is only supported on OS X 10.10 and above!'
        );
    }
    ...
    if (services.length) {
        for (var i = 0; i < services.length; i++) {
            ...
            for (var j = 0; j < service.characteristics.length; j++) {
                ...
                if (
                    characteristic.properties.indexOf(
...
blenoBindings.on('kCBMsgId18', function(args) {
    ...
    if (result) {
        var errorMessage =
            'failed to set service ' + this._attributes[attributeId].uuid;
...
blenoBindings.on('kCBMsgId20', function(args) {
    ...
    for (var i = 0; i < attWrites.length; i++) {
        ...
        this._attributes[attributeId].emit(
            'writeRequest',
            data,
            offset,
            ignoreResponse,
