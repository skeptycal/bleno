...
Gap.prototype.startAdvertisingWithEIRData = function(
    ...
    if (advertisementData.length > 31) {
        error = new Error(
            'Advertisement data is over maximum limit of 31 bytes'
        );
    ...
    if (error) {
        ...
        if (isIntelEdison || isYocto) {
            ...
            debug(
                'skipping first set of scan response and advertisement data'
            );
        } else {
