...
var READ_LE_HOST_SUPPORTED_CMD =
    OCF_READ_LE_HOST_SUPPORTED | (OGF_HOST_CTL << 10);
var WRITE_LE_HOST_SUPPORTED_CMD =
...
var LE_SET_ADVERTISING_PARAMETERS_CMD =
    OCF_LE_SET_ADVERTISING_PARAMETERS | (OGF_LE_CTL << 10);
var LE_SET_ADVERTISING_DATA_CMD =
    OCF_LE_SET_ADVERTISING_DATA | (OGF_LE_CTL << 10);
var LE_SET_SCAN_RESPONSE_DATA_CMD =
    OCF_LE_SET_SCAN_RESPONSE_DATA | (OGF_LE_CTL << 10);
var LE_SET_ADVERTISE_ENABLE_CMD =
...
Hci.prototype.init = function() {
    ...
    var deviceId = process.env.BLENO_HCI_DEVICE_ID
        ? parseInt(process.env.BLENO_HCI_DEVICE_ID)
        : undefined;
...
Hci.prototype.onSocketData = function(data) {
    ...
    if (HCI_EVENT_PKT === eventType) {
      ...
      corresponding data buffers have been freed. */
            ...
            debug(
                '\t\tLE meta event data = ' + leMetaEventData.toString('hex')
            );
            ...
            for (var i = 0; i < handles; i++) {
                ...
                debug(
                    '\t\tin progress = ' + this._handleAclsInProgress[handle]
                );
            }
            this.pushAclOutQueue();
        }
