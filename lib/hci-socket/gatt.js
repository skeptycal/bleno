    ...
    for (i = 0; i < allServices.length; i++) {
        ...
        for (j = 0; j < service.characteristics.length; j++) {
            ...
            if (
                ...
                if (
                    characteristic.secure.indexOf('writeWithoutResponse') !==
                    -1
                ) {
                    secure |= 0x04;
                }
...
Gatt.prototype.onAclStreamEnd = function() {
    ...
    for (var i = 0; i < this._handles.length; i++) {
        if (
            ...
            if (
                this._handles[i].attribute &&
                this._handles[i].attribute.emit
            ) {
        ...
        if (services.length) {
            ...
            for (i = 0; i < numServices; i++) {
                ...
                for (var j = 0; j < serviceUuid.length; j++) {
                    response[2 + i * lengthPerService + 4 + j] =
                        serviceUuid[j];
                }
            }
        } else {
    ...
    if ('2803' === uuid) {
        ...
        if (characteristics.length) {
            ...
            for (i = 1; i < characteristics.length; i++) {
                if (
                    characteristics[0].uuid.length !==
                    characteristics[i].uuid.length
                ) {
                    break;
                }
                numCharacteristics++;
            ...
            var maxCharacteristics = Math.floor(
                (this._mtu - 2) / lengthPerCharacteristic
            );
            numCharacteristics = Math.min(
                numCharacteristics,
                maxCharacteristics
            );
            ...
            for (i = 0; i < numCharacteristics; i++) {
                ...
                var characteristicUuid = new Buffer(
                    characteristic.uuid
                        .match(/.{1,2}/g)
                        .reverse()
                        .join(''),
...
Gatt.prototype.handleWriteRequestOrCommand = function(request) {
    ...
    if (handle) {
        ...
        if (
            handleProperties &&
            (withoutResponse
                ? handleProperties & 0x04
                : handleProperties & 0x08)
        ) {
            ...
            if (
                ...
                if (data.length !== 2) {
                    ...
                    if (value & 0x0003) {
                        var updateValueCallback = function(
                            valueHandle,
                            attribute
                        ) {
                            return function(data) {
                                ...
                                var useNotify =
                                    attribute.properties.indexOf('notify') !==
                                    -1;
                                var useIndicate =
                                    attribute.properties.indexOf(
                                        'indicate'
                                    ) !== -1;
                                var i;

                                if (useNotify) {
                                    var notifyMessage = new Buffer(
                                        3 + dataLength
                                    );

                                    notifyMessage.writeUInt8(
                                        ATT_OP_HANDLE_NOTIFY,
                                        0
                                    );
                                    notifyMessage.writeUInt16LE(
                                        valueHandle,
                                        1
                                    );
                                    ...
                                    debug(
                                        'notify message: ' +
                                            notifyMessage.toString('hex')
                                    );
                                    this.send(notifyMessage);

                                    attribute.emit('notify');
                                } else if (useIndicate) {
                                    var indicateMessage = new Buffer(
                                        3 + dataLength
                                    );
        ...
        if (flag === 0x00) {
            ...
            var callback = function(requestType, valueHandle) {
                return function(result) {
                    ...
                    if (ATT_ECODE_SUCCESS === result) {
                        callbackResponse = new Buffer([
                            ATT_OP_EXEC_WRITE_RESP,
                        ]);
                    } else {
                        callbackResponse = this.errorResponse(
                            requestType,
