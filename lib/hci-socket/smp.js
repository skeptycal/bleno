...
Smp.prototype.onAclStreamEncryptChange = function(encrypted) {
    if (encrypted) {
        if (this._stk && this._diversifier && this._random) {
            this.write(
                Buffer.concat([new Buffer([SMP_ENCRYPT_INFO]), this._stk])
            );
...
Smp.prototype.onAclStreamLtkNegReply = function() {
    this.write(new Buffer([SMP_PAIRING_FAILED, SMP_UNSPECIFIED]));
...
Smp.prototype.handlePairingRandom = function(data) {
    ...
    if (this._pcnf.toString('hex') === pcnf.toString('hex')) {
        ...
        this.write(Buffer.concat([new Buffer([SMP_PAIRING_RANDOM]), this._r]));
    } else {
        this.write(new Buffer([SMP_PAIRING_FAILED, SMP_PAIRING_CONFIRM]));
